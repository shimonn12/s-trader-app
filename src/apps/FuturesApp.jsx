...
const calculateFuturesMetrics = (trade) => {
    ...
    const contracts = parseFloat(parseNumberFromCommas((trade.contracts || 0).toString()));
    ...
    return {
        ...
        rDisplay: `${rMultiple.toFixed(2)}R`,
    };
};
...
const MetricCard = ({
    ...
    >
        <div className="flex justify-between items-start">
            ...
            <div className={`p-2.5 rounded-lg bg-gray-500/10 ${styles.textSecondary} flex-shrink-0 ml-3 relative z-10`}>
                <Icon size={20} />
            </div>
        </div>
        {subValue && (
...
const PerformanceTable = ({ title, data, colNameKey, t, styles, currency, icon: Icon = Target, isRTL }) => {
            ...
            <div className="overflow-x-auto">
                <table className={`w-full ${isRTL ? "text-right" : "text-left"} border-collapse`}>
                    <thead className={`${styles.inputBg} ${styles.textSecondary} text-xs uppercase font-bold`}>
                        <tr>
                            <th className={`p-3 ${isRTL ? "text-right" : "text-left"}`}>{t(colNameKey)}</th>
...
const TradeForm = ({ isOpen, onClose, onSave, editingTrade, existingStrategies, t, styles, lang, isRTL }) => {
    ...
    useEffect(() => {
        ...
        if (editingTrade) {
            setFormData({
                ...
                contracts: editingTrade.contracts || 0,
                ...
                mental: editingTrade.mental || "",
                entryParts: editingTrade.entryParts || [],
                exitParts: editingTrade.exitParts || []
            });

            // Auto-infer contracts if missing for imported trades
            if (!editingTrade.contracts && editingTrade.pnl && editingTrade.entryPrice && editingTrade.exitPrice) {
                const diff = Math.abs(parseFloat(editingTrade.entryPrice) - parseFloat(editingTrade.exitPrice));
                const pv = parseFloat(editingTrade.pointValue) || 1;
                if (diff > 0) {
                    const inferred = Math.round(Math.abs(parseFloat(editingTrade.pnl) + (parseFloat(editingTrade.fees) || 0)) / (diff * pv));
                    if (inferred > 0) {
                        setFormData(prev => ({ ...prev, contracts: inferred }));
                    }
                }
            }
    ...
    const handleSubmit = (e) => {
        ...
        const submissionData = {
            ...
            fees: formData.fees === "" ? 0 : parseNumberFromCommas((formData.fees || 0).toString()),
 ...
 * Modals
 * ========================= */
const NoteViewerModal = ({ isOpen, onClose, notes, styles, t }) => {
    if (!isOpen) return null;
    return (
...
const CalendarView = ({ trades, isRTL, t, styles, currency, lang }) => {
    ...
    return (
        <div className={`${styles.bgCard} rounded-xl shadow-xl border ${styles.border} overflow-hidden flex flex-col md:flex-row`} dir={isRTL ? "rtl" : "ltr"}>
...
const GalleryView = ({ trades, styles, t, onEdit, currency = "$", lang = "en", isRTL }) => {
                            ...
                            <div className={`p-4 flex-1 flex flex-col justify-between ${styles.bgCard} border-t ${styles.border}`}>
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className={`text-md font-bold font-mono ${trade.pnl > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                        {trade.pnl > 0 ? '+' : ''}{currency}{formatNumber ? formatNumber(trade.pnl) : trade.pnl}
                                    </div>
                                </div>
...
const MentalView = ({ trades, t, styles, isRTL, lang }) => {
    // Calculate mental state statistics
    const mentalStats = React.useMemo(() => {
        const stats = {
        ...
        trades.forEach(trade => {
            ...
            if (stats[mental]) {
                stats[mental].count++;
                stats[mental].totalPnl += trade.pnl || 0;
                if (trade.pnl > 0) stats[mental].wins++;
            }
        });
        return stats;
    }, [trades]);
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className={`text-xl font-bold ${mentalStats.green.totalPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                    {mentalStats.green.totalPnl > 0 ? '+' : ''}${mentalStats.green.totalPnl.toLocaleString()}
                                </p>
                            </div>
                            <div className="pt-4 border-t border-slate-700/50">
                                ...
                                <div className="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
                                    <div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${totalWithMental > 0 ? (mentalStats.green.count / totalWithMental) * 100 : 0}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className={`text-xl font-bold ${mentalStats.yellow.totalPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                    {mentalStats.yellow.totalPnl > 0 ? '+' : ''}${mentalStats.yellow.totalPnl.toLocaleString()}
                                </p>
                            </div>
                            <div className="pt-4 border-t border-slate-700/50">
                                <div className="flex justify-between items-center mb-1.5">
                                ...
                                <div className="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
                                    <div className="h-full bg-yellow-500 transition-all duration-500" style={{ width: `${totalWithMental > 0 ? (mentalStats.yellow.count / totalWithMental) * 100 : 0}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className={`text-xl font-bold ${mentalStats.red.totalPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                    {mentalStats.red.totalPnl > 0 ? '+' : ''}${mentalStats.red.totalPnl.toLocaleString()}
                                </p>
                            </div>
                            <div className="pt-4 border-t border-slate-700/50">
                                <div className="flex justify-between items-center mb-1.5">
                                ...
                                <div className="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
                                    <div className="h-full bg-red-500 transition-all duration-500" style={{ width: `${totalWithMental > 0 ? (mentalStats.red.count / totalWithMental) * 100 : 0}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
const ConnectionsView = ({ t, styles, isRTL, lang, manualUrl, setManualUrl, isColmexConnected, setIsColmexConnected, isSyncing, onSyncTrades, onResetColmexData, setActiveTab, username, journalType = 'futures' }) => {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            await onSyncTrades(false, localStartDate.toISOString(), localEndDate.toISOString());
        } catch (err) {
        ...
        <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                {isAddingAccount ? (
                    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => {
                                    const idVal = document.getElementById('manual-account-id-futures')?.value;
                        ...
                        <button
                            onClick={() => {
                                colmexService.exchangeCodeForToken(detectedCode, null, username).then(() => {
                                    setDetectedCode(null);
                                    if (setIsColmexConnected) setIsColmexConnected(true);
                                    colmexService.getAccounts().then(setAccounts).catch(console.error);
                                });
                            }}
                ...
                <div className="p-4 h-48 overflow-y-auto font-mono text-[11px] bg-black/10">
                    {logs.length === 0 ? (
                        ...
                        <div className="space-y-1.5">
                            {(logs || []).map((log) => (
                                <div key={log.id} className="flex gap-3 border-b border-gray-800/30 pb-1.5 last:border-0 hover:bg-white/5 transition-colors p-1 rounded">
            ...
            <div className="flex justify-end pt-4">
                <button
                    ...
                    className="px-6 py-2 rounded-lg bg-slate-600 hover:bg-slate-500 text-white font-bold text-sm transition-colors shadow-md"
                    type="button"
                >
                    {t("close")}
                </button>
...
const SettingsView = ({ settings, setSettings, t, styles, isRTL, lang, setLang, setActiveTab, onExport, onImport }) => (
    <div className={`w-full ${styles.bgCard} rounded-xl p-6 border ${styles.border} shadow-lg relative`}>
        ...
        <div className="space-y-3">
            ...
            <div className={`flex items-center justify-between p-3 rounded-lg ${styles.inputBg} border ${styles.border} ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className={`font-bold ${styles.textPrimary} text-sm flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
 ...
 * MAIN APP
 * ========================= */
export default function FuturesApp({ username: propUsername, onLogout: propOnLogout, onSwitchJournal, lang: propLang, onLangChange }) {
    ...
    const [activeTab, setActiveTab] = useState(() => {
        return localStorage.getItem('smartJournal_futures_activeTab') || "journal";
    });

    useEffect(() => {
        localStorage.setItem('smartJournal_futures_activeTab', activeTab);
    }, [activeTab]);
    ...
    // Auto-sync connection status with cloud/localStorage
    useEffect(() => {
    ...
    const handleSyncTrades = async (silent = false, customDate = null, customToDate = null) => {
        if (isSyncing) return;
        ...
        const performSyncCall = async (date, toDate = null) => {
            ...
            let finalDate = date;
            const ignoreBefore = localStorage.getItem('colmex_ignore_before_futures');
            
            if (ignoreBefore && new Date(finalDate) < new Date(ignoreBefore)) {
            ...
            const options = {
                isReal: user?.toLowerCase().includes('real') || false,
                fromDate: finalDate,
        ...
        try {
            ...
            if (syncedTrades && syncedTrades.length > 0) {
                ...
                // Identify which trades to add/update BEFORE calling setTrades
                const ignoreBefore = localStorage.getItem('colmex_ignore_before_futures');
                syncedTrades.forEach(st => {
                    ...
                    const isFutures = st.market === 'futures';
                    const exists = trades.some(t => String(t.id) === String(st.id));
                    
                    if (!exists) {
                        if (isFutures) {
                            futuresToAdd.push(st);
                            if (user) tradeService.pushTrade(user, 'futures', st);
                        }
                    } else if (isFutures) {
                        const existing = trades.find(t => String(t.id) === String(st.id));
                        ...
                        if (!hasParts && newHasParts) {
                            futuresToAdd.push(st); // This will update the existing one in state
                            if (user) tradeService.pushTrade(user, 'futures', { ...existing, ...st });
                        }
                    }
                });

                if (futuresToAdd.length > 0) {
                    setTrades(prev => {
                        const newTrades = [...prev];
                        futuresToAdd.forEach(ft => {
                            const idx = newTrades.findIndex(t => String(t.id) === String(ft.id));
                            if (idx === -1) newTrades.push(ft);
                            else newTrades[idx] = { ...newTrades[idx], ...ft };
                        });
                        return newTrades;
                    });
                }
                ...
                if (!silent) {
                    if (futuresToAdd.length > 0) {
                        alert(lang === 'he' ? `סנכרון הושלם! נוספו ${futuresToAdd.length} עסקאות חוזים חדשות.` : `Sync complete! Added ${futuresToAdd.length} futures trades.`);
                    } else if (!customDate) {
                        alert(lang === 'he' ? 'לא נמצאו עסקאות חוזים חדשות.' : 'No new futures trades found.');
                    }
                }
            } else {
                if (!silent) alert(lang === 'he' ? 'לא נמצאו עסקאות חדשות לסנכרון.' : 'No new trades found to sync.');
            }
        } catch (err) {
    ...
    const handleResetColmexData = () => {
        if (window.confirm(lang === 'he' ? 'פעולה זו תמחק את כל העסקאות שיובאו מקולמקס ותאפס את הזיכרון. עסקאות ידניות לא יימחקו. להמשיך?' : 'This will delete all trades imported from Colmex and reset the memory. Manual trades will not be affected. Continue?')) {
            ...
            localStorage.setItem('colmex_ignore_before_futures', ignoreTimestamp);
            window.dispatchEvent(new Event('colmex_stats_updated'));
            window.dispatchEvent(new Event('colmex_logs_updated'));
            window.dispatchEvent(new Event('colmex_reset_futures'));
            
            if (user) {
                tradeService.updateProfile(user, { 
                    futures_data: manualTrades,
                    futures_ignore_before: ignoreTimestamp
                });
                colmexTrades.forEach(tradeObj => {
                    tradeService.pushTrade(user, 'futures', { ...tradeObj, deleted: true, market: 'futures' });
                });
                tradeService.pushColmexReset(user, ignoreTimestamp, 'futures');
            }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            console.log('🔄 Automatic Background Sync (Futures)...');
        ...
        const handleFocus = () => {
            console.log('Focus detected, syncing Colmex (Futures)...');
            handleSyncTrades(true);
        };
    ...
    // --- Main Helpers (Defined early for component-wide use) ---
    const t = (key) => TRANSLATIONS[lang]?.[key] || key;
    const styles = getThemeStyles(settings.theme);
    const isRTL = false; // Disable RTL layout as per user request
    ...
    const handleExportTrades = () => {
        ...
        const success = excelService.exportToExcel(trades, `futures_trades_${new Date().toISOString().split('T')[0]}.xlsx`);
        if (success) {
    ...
    const handleImportTrades = async (file) => {
        try {
            const importedTrades = await excelService.parseExcel(file, importPlatform, 'futures');
            if (importedTrades && importedTrades.length > 0) {
                ...
                const uniqueNewTrades = importedTrades.map(t => {
                    const symbol = t.symbol || "";
                    let contractType = "mini"; // Default
                    if (symbol.startsWith("M") || symbol.includes("MICRO")) contractType = "micro";

                    const pv = getAutoPointValue(symbol, contractType);
                    return {
                        ...t,
                        contractType,
                        pointValue: pv || t.pointValue || 1, // Use auto, then imported, then fallback to 1
                        market: 'futures'
                    };
                ...
                if (user) {
                    tradeService.updateProfile(user, { futures_data: newTrades });

                    // Push individual trades to relay sync for other devices
                    uniqueNewTrades.forEach(trade => {
                        tradeService.pushTrade(user, 'futures', trade);
                    });
                }
    ...
    const handleUpdateGoals = (newGoalsOrUpdater) => {
        ...
        setGoals(nextGoals);
        if (user) {
            tradeService.pushGoals(user, 'futures', nextGoals);
        }
    };
    ...
    /** ===== Persistence (HYBRID: LocalStorage + Relay Sync) ===== */
    useEffect(() => {
        if (!user) {
            setHydrated(true);
            return;
        ...
        // 1. Initial Load from Local DB (Fastest)
        localDbService.loadUserData(user, 'futures').then(data => {
            if (data) {
                setTrades(Array.isArray(data.trades) ? data.trades : []);
                setSettings(data.settings || { currency: "$", theme: "dark", fontSize: 14 });
                setStartingCapital(Number.isFinite(data.startingCapital) ? data.startingCapital : 0);
            ...
            if (!localStorage.getItem('colmex_access_token')) {
                colmexService.syncWithCloud(user).then(connected => {
                    if (connected) setIsColmexConnected(true);
                });
            }
        }).catch(error => {
            console.error('Failed to load local futures data:', error);
            setHydrated(true);
        });
    ...
    const toggleSelectAll = (filteredList) => {
        if (selectedTradeIds.size === filteredList.length && filteredList.length > 0) {
    ...
    const handleBulkDelete = () => {
        ...
        if (window.confirm(lang === 'he' ? `האם למחוק ${selectedTradeIds.size} עסקאות?` : `Delete ${selectedTradeIds.size} trades?`)) {
            ...
            if (user) {
                // Update cloud backup
                tradeService.updateProfile(user, { futures_data: updatedTrades });
                // Push delete signals
                idsToDeleteArray.forEach(id => {
                    const tradeObj = trades.find(t => String(t.id) === String(id));
                    if (tradeObj) {
                        tradeService.pushTrade(user, 'futures', { ...tradeObj, deleted: true });
                    }
                });
            }
            setSelectedTradeIds(new Set());
    ...
    const handleSaveTrade = (trade) => {
        ...
        const ensured = { ...trade, ...calculateFuturesMetrics(trade), market: 'futures' };
        ...
        if (user) {
            tradeService.pushTrade(user, 'futures', ensured);
            if (ensured.image && ensured.image.length > 1000) {
                tradeService.pushImage(user, 'futures', ensured.id, ensured.image);
            }
        }
    ...
    const confirmDelete = () => {
        if (deleteConfirmId && user) {
            const tradeToDelete = trades.find(t => t.id === deleteConfirmId);
            if (tradeToDelete) {
                // Push delete signal to relay sync cloud
                tradeService.pushTrade(user, 'futures', { ...tradeToDelete, deleted: true, market: 'futures' });
            }
    ...
    const handleRestoreData = (fileOrEvent) => {
    ...
    return (
        ...
        >
            ...
            <main className="p-4 w-full space-y-4 pb-62 relative z-10">
                {/* Page Header */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <MetricCard
                        ...
                        onInputChange={async (e) => {
                            ...
                            setStartingCapital(numericVal);
                            if (user && hydrated) {
                                const dataToSave = {
                                    trades,
                                    settings,
                                    startingCapital: numericVal,
                                    goals,
                                    lang,
                                };
                                // Save to both Local DB and Cloud immediately
                                await localDbService.saveUserData(user, 'futures', dataToSave);
                                await tradeService.updateProfile(user, { futures_data: dataToSave });
                                // Push to other device relay
                                tradeService.pushCapital(user, 'futures', numericVal);
                            }
                        }}
                        icon={DollarSign}
                        styles={styles}
                ...
                {/* Tabs */}
                <div className={`flex gap-2 md:gap-4 overflow-x-auto pb-2 border-b ${styles.border} scrollbar-hide justify-between md:justify-start sticky top-[73px] ${styles.bgMain} z-40 py-2`}>
                    {[
                            ...
                            <div className={`p-4 border-b ${styles.border} flex flex-col md:flex-row md:items-center justify-between gap-4`}>
                                ...
                                <div className="shrink-0 w-full md:w-auto flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-lg font-bold flex items-center gap-2 shadow-md transition-all transform active:scale-95 group w-full md:w-auto justify-center text-xs uppercase tracking-wide"
                                        type="button"
                                    >
                                        ...
                                        {filteredTrades.map((trade) => {
                                            ...
                                            return (
                                                <React.Fragment key={trade.id}>
                                                    <tr
                                                        className={`${styles.hoverBg} ${rowTint} transition-colors text-sm group ${selectedTradeIds.has(String(trade.id)) ? 'bg-blue-500/10' : ''}`}
                                                    >
                ...
                {/* Mental Tab */}
                {activeTab === "mental" && (
                    <MentalView trades={trades} t={t} styles={styles} isRTL={isRTL} lang={lang} />
                )}
            ...
            <TradeForm
                ...
                isRTL={isRTL}
            />
            <NoteViewerModal isOpen={!!viewingNotes} onClose={() => setViewingNotes(null)} notes={viewingNotes} styles={styles} t={t} />
